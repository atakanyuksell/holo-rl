...
class TD3VisionAgent:
    def __init__(self, img_shape, collision_dim, distance_dim, rotation_dim, action_dim, device, writer=None):
        ...
        self.actor_target = Actor(img_shape, collision_dim, action_dim, gps_dim=3, distance_dim=distance_dim, rotation_dim=rotation_dim).to(device)
        ...
        self.critic1_target = Critic(img_shape, collision_dim, action_dim, gps_dim=3, distance_dim=distance_dim, rotation_dim=rotation_dim).to(device)
        self.critic2_target = Critic(img_shape, collision_dim, action_dim, gps_dim=3, distance_dim=distance_dim, rotation_dim=rotation_dim).to(device)
    ...
    def act(self, obs, noise=0.1):
        ...
        with torch.no_grad():
            action = self.actor(camera, collision, distance, rotation, gps0, gps1)
    ...
    def train(self, batch_size=64):
        ...
        with torch.no_grad():
            ...
            next_action = self.actor_target(next_camera, next_collision, next_distance, next_rotation, next_gps0, next_gps1)
            next_action = (next_action + noise).clamp(-1, 1)
            target_q1 = self.critic1_target(next_camera, next_collision, next_distance, next_rotation, next_gps0, next_gps1, next_action)
            target_q2 = self.critic2_target(next_camera, next_collision, next_distance, next_rotation, next_gps0, next_gps1, next_action)
            target_q = torch.min(target_q1, target_q2)
            target_q = reward + (1 - done) * self.gamma * target_q
        # --- Critic update ---
        current_q1 = self.critic1(camera, collision, distance, rotation, gps0, gps1, action)
        current_q2 = self.critic2(camera, collision, distance, rotation, gps0, gps1, action)
        ...
        if self._train_step % self.policy_delay == 0:
            actor_action = self.actor(camera, collision, distance, rotation, gps0, gps1)
            actor_loss = -self.critic1(camera, collision, distance, rotation, gps0, gps1, actor_action).mean()
